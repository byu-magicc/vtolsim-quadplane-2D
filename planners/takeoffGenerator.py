...
class takeoffGenerator:
    ...
    def generatePath(self,
        ...
        match pathType:
            ...
            case pathTypes.PARABOLA:
                return self.generateParabolicPath(startPosition_3D=startPosition_3D,
    ...
    def generateParabolicPath(self,
        ...
        startVel_2D = map_3D_to_2D_planeMsg(vec_3D=startVel_3D,
                                               plane_msg=self.plane)
        endVel_2D = map_3D_to_2D_planeMsg(vec_3D=endVel_3D,
                                               plane_msg=self.plane)
        
        startAccel_2D = map_3D_to_2D_planeMsg(vec_3D=startAccel_3D,
                                               plane_msg=self.plane)
        endAccel_2D = map_3D_to_2D_planeMsg(vec_3D=endAccel_3D,
                                               plane_msg=self.plane)
        ...
        startControlPoints = self.staticPathGenerator.getLocalizedControlPoints(conditions=startConditions,
                                                                                d=self.d,
                                                                                M=self.M)
        
        endControlPoints = self.staticPathGenerator.getLocalizedControlPoints(conditions=endConditions,
                                                                                d=self.d,
                                                                                M=self.M)
        ...
        totalArcLength = self.getArcLengthParabola(t1=0.0,
                                                   t2=delta_north,
                                                   c=c,
        ...
        while not withinEnd:
            ...
            current_desired_velocity = initialVelocity + currentArcLength*spatialAcceleration
            ...
            currentTime = self.getArcPositionTime(endArcLength=currentArcLength,
                                                  c=c,
                                                  vertex_2D=startPosition_2D,
                                                  alpha=1.0)
            
            #gets the actual position for this current time
            currentPosition, _ = self.getPointParabola(t=currentTime,
                                                    c=c,
                                                    vertex_2D=startPosition_2D,
                                                    alpha=1.0)
    ...
    def getPointParabola(self,
        ...
        if t == 0.0:
            ...
            tangentVector = np.array([[north_dot],
                                      [altitude_dot]])
    ...
    def getArcPositionTime(self,
        ...
        while currentArcLength < endArcLength:
            ...
            nextPosition, _ = self.getPointParabola(t=nextTime,
                                                    c=c,
                                                    vertex_2D=vertex_2D,
                                                    alpha=alpha)
...
class takeoffGenerator_old:
    ...
    def generatePath(self,
        ...
        match pathType:
            ...
            case pathTypes.PARABOLA:
                return self.generateParabolicPath(numTransitions=numTransitions,
    ...
    def generateParabolicPath(self,
        ...
        controlPoints = self.staticPathGenerator.getControlPoints_conditionsList(conditions_list=condition_list,
                                                                                 rho=self.rho,
