...
waypointData = WaypointData(
    start_waypoint=waypoint_0,
    end_waypoint=waypoint_4,
    intermediate_locations=intermediateWaypoints,
)

# instantiates the path generator
pathGenerator = waypointPathGenerator(
    waypoints=waypointData, max_curvature=1.0, degree=3, num_points_per_interval=100
)
...
quadplane = QuadplaneDynamics(ts=SIM.ts_simulation, pn_dot0=0.0, pd_dot0=0.0)
...
while sim_time < end_time and counter < numPoints:
    ...
    actual_pos_2d = np.array([[actual_pos_3d.item(0)], [actual_pos_3d.item(2)]])
