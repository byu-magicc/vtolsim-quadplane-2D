...
ax1.plot(time_array, delta_e, label='elevator')
ax1.legend()
ax1.grid(True)
ax1.set_title('Deltas')

ax2.plot(time_array, delta_t_front, label='front')
ax2.plot(time_array, delta_t_rear, label='rear')
ax2.legend()
ax2.grid(True)

ax3.plot(time_array, delta_t_thrust, label='thrust')
ax3.legend()
ax3.grid(True)
