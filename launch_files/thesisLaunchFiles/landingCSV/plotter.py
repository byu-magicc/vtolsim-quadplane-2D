...
df1=pd.read_csv('ActualPositions.csv', header=None)
df2=pd.read_csv('actualVelocities.csv', header=None)
df3=pd.read_csv('desiredPositions.csv', header=None)
df4=pd.read_csv('desiredVelocities.csv', header=None)
df5=pd.read_csv('deltas.csv', header=None)
df6=pd.read_csv('angles.csv',header=None)
df7=pd.read_csv('forcesDesired.csv',header=None)
...
forcesNorthDesired = df7[0].to_numpy()
forcesDownDesired = df7[1].to_numpy()

fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True)

#plots on the positional errors
ax1.plot(time_array, pos_north_actual, label='North Position Actual')
ax1.plot(time_array, pos_down_actual, label='Down Position Actual')
ax1.legend()
ax1.grid(True)
ax1.set_ylabel('Position (meters)')
ax1.set_title("Position")

ax2.plot(time_array, vel_north_actual, label='North Velocity Actual')
ax2.plot(time_array, vel_down_actual, label='Down Velocity Actual')
ax2.legend()
ax2.grid(True)
...
ax1.set_title("Positional Error")
...
ax1.plot(time_array, delta_t_front[:-1], label='Delta t Front')
ax1.plot(time_array, delta_t_rear[:-1], label='Delta t Rear')
ax1.legend()
ax1.grid(True)
ax1.set_title('Delta Control Inputs')

ax2.plot(time_array, delta_t_thrust[:-1], label='Delta t Thrust')
ax2.legend()
ax2.grid(True)

ax3.plot(time_array, delta_e[:-1], label='Delta elevator')
ax3.legend()
ax3.grid(True)
...
ax1.plot(time_array[:500], forcesNorthDesired[:500], label='North Desired Forces (N)')
ax1.plot(time_array[:500], forcesDownDesired[:500], label='Down Desired Forces (N)')
ax1.legend()
ax1.grid(True)
